	...
	public static function display_valid_notices() {
		...
		foreach( $messages_to_display as $message_type => $message_values ) {
			foreach( $message_values as $message_key => $message_data ) {
				...
				printf( __('%1$s : %2$s <a href="%3$s">Hide Notice</a>'), esc_html( $message_type ), wp_kses_post( $message_data['message_text'] ), add_query_arg( array( self::$dismiss_parameter => esc_attr( $message_key ) ) ) );
				printf( '</p></div>' );
			}
		}
	}
